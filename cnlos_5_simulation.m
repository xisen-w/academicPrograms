...
%Define NLOS blur kernel 2 (for 2nd
%convolution)-------------------------------------------
psf2 = definePsf2(N,M,width./range);%not sure if the constant is this

%Compute inverse psf2
fpsf2 = fftn(psf2);%Fourier Transform 
if (~isbackprop)
    invpsf2 = conj(fpsf2) ./ (abs(fpsf2).^2 + 1./snr);
else
    invpsf2 = conj(fpsf2);
end
...
tvol = ifftn(fftn(tdata).*invpsf);%重构图像 - 反卷积 
...
%Step One: Data Stucture for rou & v equality 
data2 = permute(vol,[3 2 1]);%Using vol as an input here 

%Step Two: Define volume representing voxel distance from wall
grid_z2 = repmat(linspace(0,1,M)',[1 N N]);

display('Inverting Second Phase...');
tic;

%Step Three: Scale radiometric component
if (isdiffuse)
    data2 = data2.*(grid_z.^4);
else
    data2 = data2.*(grid_z.^2);
end

%Step Four: Resample time axis and pad result
tdata2 = zeros(2.*M,2.*N,2.*N);
tdata2(1:end./2,1:end./2,1:end./2)  = reshape(mtx*data2(:,:),[M N N]);

% Step 5: Change it with new data, esp new invpsf 
tvol2 = ifftn(fftn(tdata2).*invpsf2);
tvol2 = tvol2(1:end./2,1:end./2,1:end./2);

% Step 6: Resample depth axis and clamp results
vol2  = reshape(mtxi*tvol2(:,:),[M N N]);
vol2  = max(real(vol2),0);

display('... done.');
time_elapsed = toc;

display(sprintf(['Reconstructed volume of size %d x %d x %d in %f seconds'], size(vol,3) ...
    ,size(vol,2),size(vol,1),time_elapsed));
...
function psf2 = definePsf2(U,V,constant)
% Local function to computeD NLOS blur kernel

x2 = x - linspace(-1,1,2.*U); %how to get x' & x'' 
y2 = y - linspace(-1,1,2.*U);
z2 = z - linspace(0,2,2.*V); 
[grid_z,grid_y,grid_x] = ndgrid(z2,y2,x2);

% Define PSF2
psf2 = constant*sqrt(grid_x.^2 + grid_y.^2+z0^2) - grid_z;

% Data Structure / Not Sure 
